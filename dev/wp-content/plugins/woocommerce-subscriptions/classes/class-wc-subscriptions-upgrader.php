...
class WC_Subscriptions_Upgrader {
	...
	public static function init(){
		...
		$updated_to_wc_2_0 = get_option( 'wcs_updated_to_wc_2_0', 'false' );

		if ( 'true' != $updated_to_wc_2_0 ) {
			if ( current_user_can( 'update_plugins' ) )
				add_action( 'init', __CLASS__ . '::upgrade_to_latest_wc', 1 );
			else
				add_action( 'admin_init', __CLASS__ . '::upgrade_to_latest_wc', 1 );
		}
	...
	public static function upgrade(){
		...
		if ( version_compare( self::$active_version, '1.1.3', '<' ) )
		...
		if ( version_compare( self::$active_version, '1.2.1', '<' ) )
			self::upgrade_to_version_1_2_1();

		// Fix paypal renewal orders
		if ( version_compare( self::$active_version, '1.2.2', '<' ) )
	...
	public static function upgrade_to_latest_wc() {
		...
		if ( version_compare( $current_wc_version, '2.0', '>=' ) ) {
	...
	private static function upgrade_database() {
		...
		foreach ( $orders_to_upgrade as $order_id ) {
			...
			foreach ( $order_items as $index => $order_item ) {
				...
				$item_id   = WC_Subscriptions_Order::get_items_product_id( $order_item );
				$item_meta = new WC_Order_Item_Meta( $order_item['item_meta'] );

				$subscription_interval     = ( isset( $order_subscription_intervals[$item_id] ) ) ? $order_subscription_intervals[$item_id] : 1;
				$subscription_length       = ( isset( $order_subscription_lengths[$item_id] ) ) ? $order_subscription_lengths[$item_id] : 0;
				$subscription_trial_length = ( isset( $order_subscription_trial_lengths[$item_id] ) ) ? $order_subscription_trial_lengths[$item_id] : 0;

				$item_meta->add( '_subscription_period', $order_subscription_periods[$item_id] );
			...
			foreach ( $users_subscriptions as $subscription_key => $subscription ) {
				...
				if ( isset( $subscription['completed_payments'] ) && count( $subscription['completed_payments'] ) > 0 && $order_post != null ) {
					foreach ( $subscription['completed_payments'] as $payment_date ) {
						...
						$renewal_order_id = WC_Subscriptions_Renewal_Order::generate_renewal_order( $subscription['order_id'], $subscription['product_id'], 'child' );
