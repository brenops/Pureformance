...
class WC_Subscriptions_Manager {
	...
	public static function put_subscription_on_hold( $user_id, $subscription_key ) {
		...
		if ( ! self::can_subscription_be_changed_to( 'on-hold', $subscription_key, $user_id ) ) {
			...
			$users_subscriptions = self::update_users_subscriptions( $user_id, array( $subscription_key => array( 'status' => 'on-hold' ) ) );
	...
	public static function process_subscription_payment_failure( $user_id, $subscription_key ) {
		...
		$failed_payments_exceeded = apply_filters( 'woocommerce_subscriptions_max_failed_payments_exceeded', false );

		// Have we reached the maximum failed payments allowed on the subscription
		if ( $failed_payments_exceeded || $subscription['failed_payments'] >= get_option( WC_Subscriptions_Admin::$option_prefix . '_max_failed_payments' ) ) {
	 ...
	 * @since 1.2.3
	 */
	public static function get_status_to_display( $status ) {
		...
		switch ( $status ) {
			...
			case 'suspend' : // Backward compatibility
				...
				break;
			default :
				$status_string = apply_filters( 'woocommerce_subscriptions_custom_status_string', ucfirst( $status ) );
		}

		return apply_filters( 'woocommerce_subscriptions_status_string', $status_string, $status );
	}
	...
	public static function set_next_payment_date( $subscription_key, $user_id = '', $next_payment = '' ) {
		...
		if ( $next_payment != 0 && $next_payment > time() ) {
	...
	public static function update_wp_cron_lock( $subscription_key, $lock_time, $user_id = '' ) {
		...
		$transient_key = 'wcs_blocker_' . $user_id . '_' . $subscription_key; // Allow up to 34 character for user ID, order ID & product ID (upto 1e10 values for each)
		...
		set_transient( $transient_key, 'true', $lock_time );
	}
	...
	public static function set_trial_expiration_date( $subscription_key, $user_id = '', $trial_expiration_date = '' ) {
		...
		if ( $trial_expiration_date != 0 && $trial_expiration_date > time() ) {
			...
			if ( wp_verify_nonce( $_GET['_wpnonce'], $_GET['subscription_key'] ) === false ) {
				...
				switch ( $_GET['change_subscription_to'] ) {
					case 'active' :
						...
						$status_message = __( 'resumed', WC_Subscriptions::$text_domain );
						break;
					case 'on-hold' :
	...
	public static function ajax_update_next_payment_date() {
		...
		if ( ! wp_verify_nonce( $_POST['wcs_nonce'], WC_Subscriptions::$text_domain ) ) {
			...
			if ( is_wp_error( $new_payment_timestamp ) ) {
				...
				$time_diff = $new_payment_timestamp - time();

				if ( $time_diff > 0 && $time_diff < 7 * 24 * 60 * 60 )
					$date_to_display = sprintf( __( 'In %s', WC_Subscriptions::$text_domain ), human_time_diff( time(), $new_payment_timestamp ) );
				else
	...
	public static function update_next_payment_date( $new_payment_date, $subscription_key, $user_id = '', $timezone = 'server' ) {
		...
		if ( 0 == $next_payment_timestamp ) {
		...
		} elseif ( $new_payment_timestamp <= time() ) {
			...
			self::update_wp_cron_lock( $subscription_key, $new_payment_timestamp - time(), $user_id );
	...
	public static function safeguard_scheduled_payments( $user_id, $subscription_key ) {
		...
		$transient_key     = 'wcs_blocker_' . $user_id . '_' . $subscription_key; // Allow up to 34 character for user ID, order ID & product ID (upto 1e10 values for each)
		$old_transient_key = 'block_scheduled_subscription_payments_' . $user_id . '_' . $subscription_key; // Backward compatability for version < 1.1.5

		$subscription = self::get_subscription( $subscription_key, $user_id );

		if ( get_transient( $transient_key ) == 'true' || get_transient( $old_transient_key ) == 'true' || in_array( $subscription['status'], array( 'suspended', 'on-hold', 'cancelled', 'expired', 'failed' ) ) ) {
			...
			$next_billing_timestamp = self::calculate_next_payment_date( $subscription_key, $user_id, 'timestamp', date( 'Y-m-d H:i:s' ) );

			// If there are future payments, block until then, otherwise block for 23 hours
			$transient_timeout = ( $next_billing_timestamp > 0 ) ? $next_billing_timestamp - 60 * 60 - time() : 60 * 60 * 23;
	...
	public static function touch_time( $args = array() ) {
		...
		$time_offset = time() + ( get_option( 'gmt_offset' ) * 3600 );
