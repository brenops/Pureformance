...
add_filter( 'init', 'create_paypal_standard_subscriptions', 10 );
		...
		switch( $transaction_details['txn_type'] ) {
			...
			case 'subscr_eot': // Subscription ended, either due to failed payments or expiration
				...
				if ( 1 != WC_Subscriptions_Order::get_subscription_length( $order ) ) {
					...
					if ( ( time() + 24 * 60 * 60 ) < strtotime( WC_Subscriptions_Manager::get_subscription_expiration_date( WC_Subscriptions_Manager::get_subscription_key( $order->id ), $order->customer_user ) ) )
						WC_Subscriptions_Manager::cancel_subscriptions_for_order( $order );
					else
